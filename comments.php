	...
	<section class="comments <?php echo $type == 'venue' ? "reviews-list" : ""?>">
		...
		<?php if (have_comments()) : ?>
			<ol class="commentlist">
				<?php 
					if( $type == "venue" ) {
						wp_list_comments(array('callback' => 'ootheme_review', 'max_depth' => 1));
					}
					else {
						wp_list_comments(array('callback' => 'oo_comment', 'max_depth' => 2));
					}
				?>
			</ol>
					...
					<?php 
						if( $type == "venue" ) { ?>
							<div class="areareview">
								...
								<ul>
									<?php for($i=5;$i>=1;$i--): ?>
										<li class="col-md-4"><input type="radio" <?php echo $i == 5 ? 'checked="checked"' : ''?> name="rating" value="<?php echo $i ?>" /> <span class="stars stars<?php echo $i ?>"></span></li>
									<?php endfor;?>
								</ul>
							</div>	
					<?php } ?>
