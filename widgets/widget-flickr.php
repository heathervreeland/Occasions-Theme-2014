<?php
class Ootheme_Flickr_Widget extends WP_Widget 
{
	public $cache_key;
	...
	{
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'ootheme-flickr', 'description' => 'Display a Flickr Photostream' );

		/* Widget control settings. */
		$control_ops = array( 'width' => 500);
		/* Create the widget. */
		$this->WP_Widget( 'Ootheme_Flickr_Widget-widget', 'Ootheme Flickr', $widget_ops, $control_ops);
		
		$this->cache_key = 'oothemeflickrcache';
	}
	...
	public function widget($args, $instance) 
	{
		...
		?>
		<ul class="ootheme-flickr-widget cf">
			<?php foreach ($pix as $item) : ?>
				<li class="picture">
					<a href="<?php echo $item['url']?>" title="<?php echo $item['title']?> <?php _e('by', 'ootheme')?> <?php echo $item['author_name']?>" rel="external">
						<img src="<?php echo $item['thumb']?>" alt="<?php echo $item['title']?> <?php _e('by', 'ootheme')?> <?php echo $item['author_name']?> " />
					</a>
				</li>
			<?php endforeach; ?>
		</ul>
