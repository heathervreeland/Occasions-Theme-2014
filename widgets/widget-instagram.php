<?php
class Ootheme_Instagram_Widget extends WP_Widget 
{
	public $cache_key;
	...
	{
		/* Widget settings. */
		$widget_ops = array('classname' => 'ootheme-instagram', 'description' => __('Displays latest instagrams.', 'ootheme'));

		/* Widget control settings. */
		$control_ops = array('width' => 300, 'height' => 350);
		/* Create the widget. */
		$this->WP_Widget( 'Oootheme_Instagram_Widget-widget', 'Ootheme Instagram', $widget_ops, $control_ops);
		
		$this->cache_key = 'oothemeinstagramcache';
	}
	...
	public function widget($args, $instance) {
		...
		if(isset($instance['access_token'])) {
			...
			if(!empty($images)) {
				...
				?>
				<ul class="ootheme-instagram-widget cf">
	...
	public function form($instance ) {
		$defaults = array(
			'title' => __('My instagrams', 'ootheme'),
			'login' => '',
			'pass' => '',
			'hashtag' => '',
			'dologout' => 0,
		...
		<?php if(!isset($instance['access_token'])): ?>
			<p>
				<label for="<?php echo $this->get_field_id('login'); ?>"><?php _e('Instagram username:', 'ootheme'); ?></label>
				<input id="<?php echo $this->get_field_id('login'); ?>" name="<?php echo $this->get_field_name('login'); ?>" type="text" value="" class="widefat" />
			</p>
			<p>
				<label for="<?php echo $this->get_field_id('pass'); ?>"><?php _e('Password:', 'ootheme'); ?></label>
				<input id="<?php echo $this->get_field_id('pass'); ?>" name="<?php echo $this->get_field_name('pass'); ?>" type="password" class="widefat" />
			</p>
		<?php else: ?>
			<p>
				<label for="<?php echo $this->get_field_id('hashtag'); ?>"><?php _e('Show latest public instagrams with following hashtag (without "#"; if empty, will show your recent instagrams):', 'ootheme'); ?></label>
			...
			<p>
				...
				<label for="<?php echo $this->get_field_id('logoutbutton'); ?>"><?php _e('Logged in as: ', 'ootheme'); echo $instance['login']; ?></label>
				<a id="<?php echo $this->get_field_id('logoutbutton'); ?>" class="button-secondary"><?php _e('Logout from Instagram', 'ootheme'); ?></a>
				<script>
