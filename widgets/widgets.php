 ...
 * Register Sidebars
 */
function oo_init_sidebars() {
	foreach (ootheme_get_sidebars() as $sidebar) {
		register_sidebar(
				array(
					'id' => 'ootheme-' . sanitize_title($sidebar),
					'name' => __($sidebar, 'ootheme'),
					'description' => __($sidebar, 'ootheme'),
...
add_action('init', 'oo_init_sidebars');

/**
 * Register Theme Widgets 
 */
function oo_init_widgets() {
	register_widget('Ootheme_Blog_Widget');
	register_widget('Ootheme_Tweets_Widget');
	register_widget('Ootheme_Flickr_Widget');
	register_widget('Ootheme_About_Widget');
}

add_action('widgets_init', 'oo_init_widgets');

/**
 * Image field for widgets 
 */
class Ootheme_WidgetImageField 
{
	private $image_id;
	private $src;
	private $width;
	...
	function __construct($widget = null, $image_id = 0) {
		...
		if ($uri && in_array($file, array('widgets.php')) && is_admin()) {
			...
			wp_enqueue_script('widgetimagefield', OOTHEME_URL . '/assets/js/widgets.js', array('jquery', 'jquery-ui-core', 'thickbox', 'media-upload'), false, true);
