...
function be_get_adjacent_post( $in_same_cat = false, $excluded_categories = '', $previous = true, $taxonomy = 'category' ) {
	...
	$where = apply_filters( "get_{$adjacent}_post_where", $wpdb->prepare("WHERE p.post_date $op %s AND p.post_type = %s AND p.post_status = 'publish'", $current_post_date, $post->post_type), $in_same_cat);
 ...
 * @param string $taxonomy Optional. Which taxonomy to use.
 */
function be_previous_post_link($format='&laquo; %link', $link='%title', $in_same_cat = false, $excluded_categories = '', $taxonomy = 'category', $echo = true) {
	return be_adjacent_post_link($format, $link, $in_same_cat, $excluded_categories, true, $taxonomy, $echo);
}
 ...
 * @param string $taxonomy Optional. Which taxonomy to use.
 */
function be_next_post_link($format='%link &raquo;', $link='%title', $in_same_cat = false, $excluded_categories = '', $taxonomy = 'category', $echo = true) {
	return be_adjacent_post_link($format, $link, $in_same_cat, $excluded_categories, false, $taxonomy, $echo);
}
 ...
 * @param string $taxonomy Optional. Which taxonomy to use.
 */
function be_adjacent_post_link($format, $link, $in_same_cat = false, $excluded_categories = '', $previous = true, $taxonomy = 'category', $echo= true) {
	...
	if(!$echo)
		return array("title" => $title, "link" => get_permalink($post->ID));
	else
