 ...
 * @param int $limit
 */
function oo_tweets($limit = 1) {
    $messages = oo_get_tweets(oo_get_twitter_name(), $limit);
    ...
    foreach ( $messages as $message ) {
        ...
        $msg = oo_twitter_hyperlinks($msg);
        $msg = oo_twitter_users($msg);
 ...
 * @return array
 */
function oo_get_tweets($username, $limit = 1) {
    if (!$username) {
        return false;
    }
    ...
    $pie = new SimplePie('http://api.twitter.com/1/statuses/user_timeline.rss?screen_name=' . $username . '&count=' . $limit, sys_get_temp_dir());
 ...
 * @return string 
 */
function oo_twitter_hyperlinks($text) {
 ...
 * @return string 
 */
function oo_twitter_users($text) {
 ...
 * @return string
 */
function oo_get_twitter_name() {
    preg_match('~([^/]+)$~si', oo_get_option('twi'), $matches);
    return $matches[1];
}
 ...
 * @param string $type 
 */
function oo_share($type = 'fb') {
    echo oo_get_share($type);
}
 ...
 * @return string
 */
function oo_get_share($type = 'fb', $permalink = false, $title = false) {
    if (!$permalink) {
    ...
    switch ($type) {
        ...
        case 'pin':
            ...
            break;
        case 'pinit':
            return "javascript:void(showPinterest('post-".get_the_ID()."'))";
            break;            
        default:
            return '';
    }
 ...
 * @return string
 */
function oo_get_twitter() {
	return oo_get_option('twi') ?  'http://twitter.com/#!/' . oo_get_option('twi') : '';
}
function oo_twitter() {
	echo oo_get_twitter();
}