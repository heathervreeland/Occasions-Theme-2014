...
$order = false;
$ajaxRefresh = false;
if(isset($_GET['ouid']) && !Cart66Session::get('Cart66PayPalStandardOUID')) {
...
elseif(Cart66Session::get('Cart66PayPalStandardOUID')) {
  $order = new Cart66Order();
  $order->loadByOuid(Cart66Session::get('Cart66PayPalStandardOUID'));
  if(empty($order->id) || $order->status == 'paypal_pending') {
    $ajaxRefresh = true;
  }
  else {
    $url = add_query_arg('ouid', Cart66Session::get('Cart66PayPalStandardOUID'), Cart66Common::getCurrentPageUrl());
    Cart66Session::drop('Cart66PayPalStandardOUID');
    wp_redirect($url);
    exit;
  }
}
...
<?php else: ?>
  ...
  <input type="hidden" name="ouid" id="ouid" value="<?php echo Cart66Session::get('Cart66PayPalStandardOUID'); ?>" />
